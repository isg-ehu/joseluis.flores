...
RunFile           = "Resultados.20160829/Full.Database.03.csv"
...
FILE_RESULTS=paste("ChiMerge.Results.",DATE,",",TIME,".csv")
...
DM       = "Modified Chi2 (ChiMerge)"
numFILES = length(TrainFiles)
DATA     = array(dim=c(numFILES,13))
RESULTS  = data.frame(DATA)
colnames(RESULTS)=c("Date","Time","Algorithm","TrainDatabase","TestDatabase","TruePositive","TrueNegative","FalsePositive","FalseNegative","Accuracy","Intervals","Discretizable","NonDiscretizableVars")
...
for(file in 1:length(TrainFiles))
{
  ...
  ## Discretization method 1: ChiMerge
  alpha=0.05
  discretization=chiM(data[c(vars,classPos)],alpha) 
  cp_list=discretization$cutp
  dataDISCRETIZED[c(vars,classPos)]=discretization$Disc.data
  
  logging(LogPath,FILE,"Phase 2","Step 7: ChiMerge Significance Level",toString(alpha))
  ...
  for(var in vars)
  {
    ...
    if (length(ZEROS)>0 & is.numeric(ZEROS)) {
      ...
      logging(LogPath,FILE,"Phase 2",paste("Step 9: ChiMerge Valid Variable"),toString(var))
    } else {
      tmpVars[var]=var
      print("CRACK")
      RESULTS$Discretizable[file]="No"
      NonDiscretizable=paste(NonDiscretizable,toString(var))
    }
    i=i+1
    logging(LogPath,FILE,"Phase 2",paste("Step 9: ChiMerge Discretization Sequence", toString(var)),toString(ZEROS))
  }
  RESULTS$Intervals[file]=Intervals
  RESULTS$NonDiscretizableVars[file]=NonDiscretizable 
  tmpVars2=unique(tmpVars)
  removeVars=tmpVars2[which(tmpVars2>0)]
  numRemovableVars=length(removeVars)
  print(numRemovableVars)
  ...
  print(length(removeVars))
  if (numRemovableVars>0 ) TRAIN_X_DISCRETIZED = TRAIN_X_DISCRETIZED[-removeVars]
  TRAIN_Y_DISCRETIZED = dataDISCRETIZED    [, classPos]

  TEST_X_DISCRETIZED  = dataTestDISCRETIZED[,-classPos]
  if (numRemovableVars>0 ) TEST_X_DISCRETIZED  = TEST_X_DISCRETIZED [-removeVars]
  TEST_Y_DISCRETIZED  = dataTestDISCRETIZED[,classPos]
  
  ## Generar los ARFFs
  TRD<-data.frame(TRAIN_X_DISCRETIZED,TRAIN_Y_DISCRETIZED)
  write.arff(TRD,paste(TrainFiles[file],".ChiMerge.NEW.DISCRETIZED.arff",sep="")) 
  TSD<-data.frame(TEST_X_DISCRETIZED,TEST_Y_DISCRETIZED)
  colnames(TSD)<-colnames(TRD)
  write.arff(TSD,paste(TestFiles[file],".ChiMerge.NEW.DISCRETIZED.arff",sep=""))    
  ...
  RESULTS$Accuracy     [file]=(CMTest[1]+CMTest[4])/(CMTest[1]+CMTest[2]+CMTest[3]+CMTest[4])
}
