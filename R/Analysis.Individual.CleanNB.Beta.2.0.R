...
#  data                 = read.arff(paste(path,test_file,sep=""))
  ...
  ########################################################################################  
  train_plus_min_x=min(X_train_PLUS)-h_plus
  train_plus_max_x=max(X_train_PLUS)+h_plus
  train_minus_min_x=min(X_train_MINUS)-h_minus
  train_minus_max_x=max(X_train_MINUS)+h_minus
  ...
  if (length(X_Zeros)>0) {
    ResultadosACC<-acc(X_train,Y_train,ZEROS)
    print(ResultadosACC)
    ...
    strLine=paste(strLine,toString(ResultadosACC)) # LAUC
...
DatabaseFile      = "Exp.Configuration.27.csv"
...
sink("R.log", append=FALSE, split=FALSE)
