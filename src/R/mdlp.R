...
FILE_RESULTS=paste("MDLP.Results.",DATE,",",TIME,".csv")
...
for(file in 1:length(TrainFiles)) #1:length(TrainFiles)
{
  ...
  ## Discretization method 1: MDLP
  discretization=mdlp(data[c(vars,classPos)])  
  ...
  for(var in vars)
  {
    ...
    if (length(ZEROS)>0 & is.numeric(ZEROS)) {
      ...
      logging(LogPath,FILE,"Phase 2",paste("Step 9: MDLP Valid Variable"),toString(var))
    } else {
      tmpVars[var]=var
      print("CRACK")
      isValid=FALSE
    ...
    i=i+1
    logging(LogPath,FILE,"Phase 2",paste("Step 9: MDLP Discretization Sequence", toString(var)),toString(ZEROS))
  }
