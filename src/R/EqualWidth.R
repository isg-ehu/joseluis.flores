...
FILE_RESULTS=paste("EqualWidth.Results.",DATE,",",TIME,".csv")
...
for(file in 1:length(TrainFiles))
{
  ...
  ## Discretization method 1: EqualWidth
  isValid=TRUE
  cp_list=list()
  ...
  for(var in vars) 
  {
    print(round(round(1+log2(length(data[,var])))))
    discretization=equalWidth(data[,var],binsEstimator(1,length(data[,var])))
    ...
    if (length(ZEROS)>0) {
      ...
      logging(LogPath,FILE,"Phase 2",paste("Step 9: EqualWidth Valid Variable"),toString(var))
      logging(LogPath,FILE,"Phase 2",paste("Step 9: EqualWidth Discretization Sequence", toString(var)),toString(ZEROS))      
    } else {
      idValid = FALSE
  ...
  write.arff(TRD,paste(TrainFiles[file],".EW.NEW.DISCRETIZED.arff",sep="")) 
  TSD<-data.frame(TEST_X_DISCRETIZED,TEST_Y_DISCRETIZED)
  colnames(TSD)<-colnames(TRD)
  write.arff(TSD,paste(TestFiles[file],".EW.NEW.DISCRETIZED.arff",sep=""))    
