...
for(file in 1:length(TrainFiles))
{
  ...
  for(var in vars)
  {
    ...
    print(length(ZEROS))
    if (length(ZEROS)<=0) print("Not Discretizable")
    ...
    i=i+1
    logging(LogPath,FILE,"Phase 2",paste("Step 9: ChiM Discretization Sequence", toString(var)),toString(ZEROS))
  }
  ...
  write.arff(TRD,paste(TrainFiles[file],".CHI2MERGE.NEW.DISCRETIZED.arff",sep="")) 
  TSD<-data.frame(TEST_X_DISCRETIZED,TEST_Y_DISCRETIZED)
  colnames(TSD)<-colnames(TRD)
  write.arff(TSD,paste(TestFiles[file],".CHI2MERGE.NEW.DISCRETIZED.arff",sep=""))    
  ...
  CMTest<-table(predict(NB$finalModel,dataTestDISCRETIZED[,-classPos])$class,dataTestDISCRETIZED[,classPos])
