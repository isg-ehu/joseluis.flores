...
#    print(paste("Iteration:",toString(i)))
    ...
    if (length_i_PLUS>0 ) {
      for(point in 1:length(X_i_PLUS)) {
        A_PLUS <- A_PLUS+FACTOR_PLUS*(-1)
        B_PLUS <- B_PLUS+FACTOR_PLUS*(2*X_PLUS[X_i_PLUS[point]])
        C_PLUS <- C_PLUS+FACTOR_PLUS*(h_plus*h_plus-X_PLUS[X_i_PLUS[point]]*X_PLUS[X_i_PLUS[point]])
      }
      df$df.a.plus[i]=A_PLUS/(h_plus*NI_PLUS)
      df$df.b.plus[i]=B_PLUS/(h_plus*NI_PLUS)
      df$df.c.plus[i]=C_PLUS/(h_plus*NI_PLUS)
    }
    ...
    if ( length_i_MINUS > 0 ) {
      for(point in 1:length(X_i_MINUS)) {
        A_MINUS <- A_MINUS+FACTOR_MINUS*(-1)
        B_MINUS <- B_MINUS+FACTOR_MINUS*(2*X_MINUS[X_i_MINUS[point]])
        C_MINUS <- C_MINUS+FACTOR_MINUS*(h_minus*h_minus-X_MINUS[X_i_MINUS[point]]*X_MINUS[X_i_MINUS[point]])
      }
      df$df.a.minus[i]=A_MINUS/(h_minus*NI_MINUS)
      df$df.b.minus[i]=B_MINUS/(h_minus*NI_MINUS)
      df$df.c.minus[i]=C_MINUS/(h_minus*NI_MINUS)
    }
