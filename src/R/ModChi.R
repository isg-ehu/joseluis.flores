...
FILE_RESULTS=paste("ModChi2.Results.",DATE,",",TIME,".csv")
...
for(file in 1:length(TrainFiles))
{
  ...
  ## Discretization method 1: modChi2
  alpha=0.05
  discretization=modChi2(data[c(vars,classPos)],alpha)  
  cp_list=discretization$cutp
  dataDISCRETIZED[c(vars,classPos)]=discretization$Disc.data
  
  logging(LogPath,FILE,"Phase 2","Step 7: Modified ChiSquare Significance Level",toString(alpha))
  ...
  for(var in vars)
  {
    ...
    if (length(ZEROS)>0 & is.numeric(ZEROS)) {
      ...
      logging(LogPath,FILE,"Phase 2",paste("Step 9: Modified ChiSquare Valid Variable"),toString(var))
    } else {
      tmpVars[var]=var
      print("CRACK")
    ...
    i=i+1
    logging(LogPath,FILE,"Phase 2",paste("Step 9: Modified ChiSquare Discretization Sequence", toString(var)),toString(ZEROS))
  }
  ...
  write.arff(TRD,paste(TrainFiles[file],".MODCHI2.EF.NEW.DISCRETIZED.arff",sep="")) 
  TSD<-data.frame(TEST_X_DISCRETIZED,TEST_Y_DISCRETIZED)
  colnames(TSD)<-colnames(TRD)
  write.arff(TSD,paste(TestFiles[file],".MODCHI2.EF.NEW.DISCRETIZED.arff",sep=""))    
