...
FILE_RESULTS=paste("Chi2.Results.",DATE,",",TIME,".csv")
...
for(file in 1:length(TrainFiles))
{
  ...
  ## Discretization method 1: Chi2
  alp=0.5
  del=0.05
  discretization=chi2(data[c(vars,classPos)],alp,del)  
  cp_list=discretization$cutp
  dataDISCRETIZED[c(vars,classPos)]=discretization$Disc.data
  
  logging(LogPath,FILE,"Phase 2","Step 7: Chi2 Significance Level",toString(alp))
  logging(LogPath,FILE,"Phase 2","Step 8: Chi2 Inconsistence"     ,toString(del))
  ...
  for(var in vars)
  {
    ...
    i=i+1
    logging(LogPath,FILE,"Phase 2",paste("Step 9: Chi2 Discretization Sequence", toString(var)),toString(ZEROS))
  }
  ...
  write.arff(TRD,paste(TrainFiles[file],".CHI2.NEW.DISCRETIZED.arff",sep="")) 
  TSD<-data.frame(TEST_X_DISCRETIZED,TEST_Y_DISCRETIZED)
  colnames(TSD)<-colnames(TRD)
  write.arff(TSD,paste(TestFiles[file],".CHI2.NEW.DISCRETIZED.arff",sep=""))    
  ...
  RESULTS$Accuracy     [file]=100.0*(CMTest[1]+CMTest[4])/(CMTest[1]+CMTest[2]+CMTest[3]+CMTest[4])
}
