...
FILE_RESULTS=paste("EqualFrequency.Results.",DATE,",",TIME,".csv")
...
for(file in 1:length(TrainFiles))
{
  ...
  ## Discretization method 1: EqualFrequency
  isValid=TRUE
  cp_list=list()
  ...
  for(var in vars) 
  {
    discretization=equalFrequency(data[,var],binsEstimator(1,length(data[,var])))
#    discretization=equalFrequency(data[,var],4)    
#    discretization=equalFrequency(data[,var],round(1+log2(length(data[,var]))))
    ZEROS=discretization$cutp
    if (length(ZEROS)>0) {
      dataDISCRETIZED    [,var] = discretize(data    [,var],ZEROS)
      ...
      logging(LogPath,FILE,"Phase 2",paste("Step 9: EqualFrequency Valid Variable"),toString(var))
      logging(LogPath,FILE,"Phase 2",paste("Step 9: EqualFrequency Discretization Sequence", toString(var)),toString(ZEROS))      
    } else {
      idValid = FALSE
  ...
  write.arff(TRD,paste(TrainFiles[file],".EF.NEW.DISCRETIZED.arff",sep="")) 
  TSD<-data.frame(TEST_X_DISCRETIZED,TEST_Y_DISCRETIZED)
  colnames(TSD)<-colnames(TRD)
  write.arff(TSD,paste(TestFiles[file],".EF.NEW.DISCRETIZED.arff",sep=""))  
