...
FILE_RESULTS=paste("extendChi2.Results.",DATE,",",TIME,".csv")
...
for(file in 1:length(TrainFiles))
{
  ...
  ## Discretization method 1: modextendChi2
  alp=0.05
  discretization=extendChi2(data[c(vars,classPos)],alp)  
  cp_list=discretization$cutp
  dataDISCRETIZED[c(vars,classPos)]=discretization$Disc.data
  
  logging(LogPath,FILE,"Phase 2","Step 7: Extended ChiSquare Significance Level",toString(alp))
  ...
  for(var in vars)
  {
    ...
    if (length(ZEROS)>0 & is.numeric(ZEROS)) {
      ...
      logging(LogPath,FILE,"Phase 2",paste("Step 9: Extended ChiSquare Valid Variable"),toString(var))
    } else {
      tmpVars[var]=var
      print("CRACK")
      isValid=FALSE
    ...
    i=i+1
    logging(LogPath,FILE,"Phase 2",paste("Step 9: Extended ChiSquare Discretization Sequence", toString(var)),toString(ZEROS))
  }
  ...
  if (length(removeVars)>0) print(removeVars)
  ...
  write.arff(TRD,paste(TrainFiles[file],".ExtendChi2.NEW.DISCRETIZED.arff",sep="")) 
  TSD<-data.frame(TEST_X_DISCRETIZED,TEST_Y_DISCRETIZED)
  colnames(TSD)<-colnames(TRD)
  write.arff(TSD,paste(TestFiles[file],".ExtendChi2.EF.NEW.DISCRETIZED.arff",sep=""))    
