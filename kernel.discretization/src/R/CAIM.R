...
FILE_RESULTS=paste("CAIM.Results.",DATE,",",TIME,".csv")
...
for(file in 1:length(TrainFiles))
{
  ...
  ## Discretization method 1: CAIM
  discretization=disc.Topdown(data[c(vars,classPos)],method=1)  
  ...
  for(var in vars)
  {
    ...
    if (length(ZEROS)>0 & is.numeric(ZEROS)) {
      ...
      logging(LogPath,FILE,"Phase 2",paste("Step 9: CAIM Valid Variable"),toString(var))
    } else {
      tmpVars[var]=var
      print("CRACK")
    ...
    i=i+1
    logging(LogPath,FILE,"Phase 2",paste("Step 9: CAIM Discretization Sequence", toString(var)),toString(ZEROS))
  }
  ...
  write.arff(TRD,paste(TrainFiles[file],".CAIM.NEW.DISCRETIZED.arff",sep="")) 
  TSD<-data.frame(TEST_X_DISCRETIZED,TEST_Y_DISCRETIZED)
  colnames(TSD)<-colnames(TRD)
  write.arff(TSD,paste(TestFiles[file],".CAIM.NEW.DISCRETIZED.arff",sep=""))    
